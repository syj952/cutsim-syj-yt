...
public:
    ...
    ///added hust
      ...
      //根据力位置查找最近节点的方法
      std::vector<Octnode*> findNearestNodesForForces(const std::vector<AptCutterVolume::ForceData>& forceData,std::vector<Octnode*>& boundarynode) const;
    ///added hust
    void diff_volume_cuda( const CylCutterVolume* vol );
    void diff_volume_blade( const Volume* vol );
    void diff_volume_blade_cuda( const AptCutterVolume* volume );
