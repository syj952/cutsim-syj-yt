...
//读取切削刃点集
void AptCutterVolume::readTestPointsFromFile(const std::string& filename) {
...
bool AptCutterVolume::checkConvexity(const GLVertex& a, const GLVertex& b,
...
void AptCutterVolume::calculatePosition_balde() {
    ...
    for (auto& point : original_blade_points) {
        ...
        GLVertex point_orign = point.rotateABC(0, 0, tool_angle) + center; // b
        GLVertex point_offset = point.rotateABC(0, 0, -step + tool_angle) + center - GLVertex(dx, dy, dz); // c
        GLVertex center_offset = center_orign - GLVertex(dx, dy, dz); // d

        // 检查四边形凸性，若为凹则交换b和c的顺序
        bool is_convex = checkConvexity(center_orign, point_orign, point_offset, center_offset);
    ...
    // 首先找到所有点的最小和最大坐标值
    GLVertex min_coord = blade_points.empty() ? GLVertex() : blade_points[0];
    GLVertex max_coord = blade_points.empty() ? GLVertex() : blade_points[0];

    for (const auto& point : blade_points) {
    ...
    min_coord.y = min_coord.y - 0.1 * (max_coord.y - min_coord.y);
    min_coord.z = min_coord.z - 0.1 * (max_coord.z - min_coord.z);

    max_coord.x = max_coord.x + 0.1 * (max_coord.x - min_coord.x);
    max_coord.y = max_coord.y + 0.1 * (max_coord.y - min_coord.y);
...
void AptCutterVolume::calculateForceData(double step,double cube_resolution) {

    force_map.clear();

    for (const auto& pair : cut_h) {
        int inside_index = pair.first;    // 获取inside_index
        float force_cut_h = pair.second; // 获取cut_h值

        GLVertex blade_bottom = blade_points[inside_index+1];
        GLVertex blade_up = blade_points[inside_index+5];
        float force_cut_w =std::abs(blade_up.z-blade_bottom.z);

        ForceData fd;

        // 计算径向力 (force_vr)
        fd.force_vr = GLVertex(center.x-blade_bottom.x,center.y-blade_bottom.y,0);
        // 归一化处理
        double length = fd.force_vr.norm();
        if (length > 0) {
            fd.force_vr = fd.force_vr * (1.0 / length);
        }

        // 计算轴向力 (force_va)
        fd.force_va = blade_bottom-blade_up;
        // 归一化处理
        length = fd.force_va.norm();
        if (length > 0) {
            fd.force_va = fd.force_va * (1.0 / length);
        }

        // 计算切向力 (force_vt)
        fd.force_vt = fd.force_vr.cross(fd.force_va);

        // 计算合力 (force_value)
        fd.force_value = GLVertex(-(K_re*force_cut_h+K_rc*force_cut_h)*force_cut_w,
                                  -(K_te*force_cut_h+K_tc*force_cut_h)*force_cut_w,
                                  (K_ae*force_cut_h+K_ac*force_cut_h)*force_cut_w);
//        fd.force_value = GLVertex(force_cut_h*1000,
//                                  0,
//                                  0);
...
//    std::cout << "z\tFx\tFy\tFz" << std::endl;
//    for (const auto& pair : force_map) {
//        const ForceData& fd = pair.second;
//        std::cout << pair.first << "\t"
//                  << fd.force_value.x << "\t"
//                  << fd.force_value.y << "\t"
//                  << fd.force_value.z << std::endl;
//    }
...
void AptCutterVolume::calculateTotalForce(){
    ...
    if (it != cutnum_angle_force_map.end()) {
        ...
        GLVertex total_force(0, 0, 0);
        collected_force_data.clear();
        for (const auto& inner_pair : angle_force_map) {
            double  force_cutnum= inner_pair.first;  // 第二层键（float）
            const auto& force_map_2 = inner_pair.second;  // ForceData 数据
                ...
                for (const auto& force_pair : force_map_2) {
                    ...
                    total_force.y += -(fd.force_value.x*fd.force_vr.y+fd.force_value.y*fd.force_vt.y+fd.force_value.z*fd.force_va.y);
                    total_force.z += -(fd.force_value.x*fd.force_vr.z+fd.force_value.y*fd.force_vt.z+fd.force_value.z*fd.force_va.z);
                }
        }
        // 存储每个角度的合力
        angle_total_force[tool_angle] = total_force;
        //qDebug() << "total_force.x:" << total_force.x;
    }
    else {
...
void AptCutterVolume::plotForceCurves(const std::map<double, GLVertex>& angle_total_force) {
    // 创建非模态对话框
    QDialog *dialog = new QDialog(QApplication::activeWindow());
    dialog->setAttribute(Qt::WA_DeleteOnClose);
    dialog->resize(850, 650);  // 增加宽度以容纳图例
...
void AptCutterVolume::PlotForces(double cube_resolution) {

    // 输出数据用于绘图
    std::cout << "tool_angle (rad)\tFx\tFy\tFz" << std::endl;
    ...
    // 这里可以添加绘图代码
    // 例如使用opengl绘图库
    plotForceCurves(angle_total_force);
}

void AptCutterVolume::plotVibrationCurves() {
    // 合并收集x/y方向的位移、速度、加速度数据
    std::map<double, GLVertex> qxqy_data;    // 位移数据（x和y分量）
    std::map<double, GLVertex> dqxqy_data;   // 速度数据（x和y分量）
    std::map<double, GLVertex> ddqxqy_data;  // 加速度数据（x和y分量）

    // 收集位移数据（x和y）
    for (const auto& pair : q_x) {
        double angle = pair.first;
        double qx = pair.second;
        double qy = q_y[angle];  // 获取y方向位移值
        qxqy_data[angle] = GLVertex(qx, qy, 0);  // x分量存x属性，y分量存y属性
    }

    // 收集速度数据（x和y）
    for (const auto& pair : dot_q_x) {
        double angle = pair.first;
        double dqx = pair.second;
        double dqy = dot_q_y[angle];  // 获取y方向速度值
        dqxqy_data[angle] = GLVertex(dqx, dqy, 0);
    }

    // 收集加速度数据（x和y）
    for (const auto& pair : ddot_q_x) {
        double angle = pair.first;
        double ddqx = pair.second;
        double ddqy = ddot_q_y[angle];  // 获取y方向加速度值
        ddqxqy_data[angle] = GLVertex(ddqx, ddqy, 0);
    }

    // 辅助函数：创建显示两个分量的窗口
    auto createCurveWindow = [](const std::map<double, GLVertex>& data, const QString& title, const QString& unit) {
        QDialog *dialog = new QDialog(QApplication::activeWindow());
        dialog->setAttribute(Qt::WA_DeleteOnClose);
        dialog->resize(850, 650);

        QVBoxLayout *layout = new QVBoxLayout(dialog);
        layout->setContentsMargins(10, 5, 10, 5);  // 减少对话框边距

        // 标题标签（缩小字体，取消粗体）
        QLabel *titleLabel = new QLabel(title);
        titleLabel->setFont(QFont("Arial", 10));  // 原12号粗体改为10号常规
        titleLabel->setContentsMargins(0, 2, 0, 2);  // 减少标题上下边距
        layout->addWidget(titleLabel);
        layout->setStretchFactor(titleLabel, 0);  // 标题固定大小

        // 绘图部件
        VibrationCurveWidget *widget = new VibrationCurveWidget(nullptr, unit);
        widget->setData(data);
        layout->addWidget(widget);
        layout->setStretchFactor(widget, 1);  // 曲线区域占满剩余空间
        ...
        auto addLegendItem = [legendLayout](const QString& text, const QColor& color) {
            ...
            label->setText(QString("<span style='color:%1;'>■</span> %2").arg(color.name()).arg(text));
            legendLayout->addWidget(label);
        };
        addLegendItem("X 分量", Qt::red);
        addLegendItem("Y 分量", Qt::green);
        legendLayout->addStretch();
    ...
    // 创建三个窗口，分别显示位移、速度、加速度的x/y分量
    createCurveWindow(qxqy_data, "振动参数 - 位移 (q_x & q_y)", "mm");
    createCurveWindow(dqxqy_data, "振动参数 - 速度 (dot_q_x & dot_q_y)", "mm/s");
    createCurveWindow(ddqxqy_data, "振动参数 - 加速度 (ddot_q_x & ddot_q_y)", "mm/s²");

    // 输出数据用于验证（保持原有逻辑）
    std::cout << "Rotation Angle\tq_x\tdot_q_x\tddot_q_x" << std::endl;
    for (const auto& pair : qxqy_data) {
        std::cout << pair.first << "\t"
                  << pair.second.x << "\t"
                  << dqxqy_data[pair.first].x << "\t"
                  << ddqxqy_data[pair.first].x << std::endl;
    }
    // 输出数据用于验证（保持原有逻辑）
    std::cout << "Rotation Angle\tq_y\tdot_q_y\tddot_q_y" << std::endl;
    for (const auto& pair : qxqy_data) {
        std::cout << pair.first << "\t"
                  << pair.second.y << "\t"
                  << dqxqy_data[pair.first].y << "\t"
                  << ddqxqy_data[pair.first].y << std::endl;
    }
}
