...
#endif
{
    ...
    case DRILL:
        ...
        case APT: {
            glTranslated(x, y, z);
            AptCutterVolume* aptCutter = (AptCutterVolume*)cutter;

            // 绘制每个分段
            for (const auto& seg : aptCutter->segments) {
                glPushMatrix();
                glTranslated(0.0, 0.0, seg.z_start);

                switch(seg.type) {
                case 0:
                    glColor3f(TOOL_FLUTE_COLOR);
                    // 绘制圆柱体
                    gluCylinder(quad, seg.radius1, seg.radius1, seg.length, 30, 1);
                    // 绘制底部圆盘
                    gluQuadricOrientation(quad, GLU_INSIDE);
                    gluDisk(quad, 0.0, seg.radius1, 30, 1);
                    // 绘制顶部圆盘
                    glTranslated(0.0, 0.0, seg.length);
                    gluQuadricOrientation(quad, GLU_OUTSIDE);
                    gluDisk(quad, 0.0, seg.radius1, 30, 1);
                    break;
                ...
                case 1: {
                    glColor3f(TOOL_FLUTE_COLOR);
                    // 绘制球体
                    gluSphere(quad, seg.radius1, 30, 30);
                    break;
                }
                ...
                case 2:
                    glColor3f(TOOL_FLUTE_COLOR);
                    // 绘制圆锥体
                    gluCylinder(quad, seg.radius1, seg.radius2, seg.length, 30, 1);
                    // 绘制底部圆盘
                    gluQuadricOrientation(quad, GLU_INSIDE);
                    gluDisk(quad, 0.0, seg.radius1, 30, 1);
                    // 绘制顶部圆盘
                    glTranslated(0.0, 0.0, seg.length);
                    gluQuadricOrientation(quad, GLU_OUTSIDE);
                    gluDisk(quad, 0.0, seg.radius2, 30, 1);
                    break;
                }
